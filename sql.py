...
#=========================================== 是否在数据库里
def _isInDatabase(cursor,table,data,division=1,key=None):
	if division > 1 and key:
		if isdigit(data[key]):
			table = table + str(data[key]%division)
		else:
			table = table + str(int(str(data[key])[-16:].upper(),16)%division)
...
#用于判断数据库里是否已经有该数据,可以传列表
#如果，有分表的话，要传入用于分表的字段的key，该字段一般是数字或md5值
def isInDatabase(cursor,table,data,division=1,key=None):
	if isinstance(data,list):
		return _isInDatabaseList(cursor,table,data,division,key)
	else:
		return _isInDatabase(cursor,table,data,division,key)

#sql防注入
def has_bad_sql(args):
	sql_key = [" and ","execute ","update ","count ",
	"chr ","mid ","master ","truncate ",
	" char","","declare ","select ","create ",
	"delete ","insert ","'",'"'," or ","="]
...
def data2redis(data,rc,queue,table,type,key,division=1):
	...
	if division == 1:
		...
		if key == "md5":
			...
			md5 = tb.util.GetMd5(data[key])
		result["table"] = table + str(int(str(md5)[-16:].upper(),16)%division)
